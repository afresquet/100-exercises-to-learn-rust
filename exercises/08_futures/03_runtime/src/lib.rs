    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let first = tokio::spawn(reply_fixed(first, reply.clone()));
    let second = tokio::spawn(reply_fixed(second, reply));
    let _ = tokio::join!(first, second);
}

async fn reply_fixed<T>(tcp_listener: TcpListener, reply: Arc<T>) -> anyhow::Result<()>
where
    T: Display + Send + Sync + 'static,
{
    loop {
